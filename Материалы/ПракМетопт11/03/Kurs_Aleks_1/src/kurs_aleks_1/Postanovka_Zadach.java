...
public class Postanovka_Zadach extends JApplet {
 ...
 Panel_Autor jPanel_Autor = new Panel_Autor();
  ...
  private void jbInit() throws Exception {
     ris = getImage(getCodeBase(), "risun.gif");
  ...
  class  Panel_Autor extends JPanel {
      public void paintComponent(Graphics  resh) {
        super.paintComponent( resh);
         resh.drawImage(ris, 1,1, this);
        resh.setColor(new Color(157, 187, 255));
        resh.drawRoundRect(5,27,700,150,20,20);
      resh.setColor(Color.white);
       resh.fillRoundRect(6,29,699,148,20,20);


   resh.setColor(new Color(60, 0, 60));
   resh.setFont(new java.awt.Font("Dialog", 1, 20));
   resh.drawString("Автор программы - студент ПМ - 43 Полушин Александр. ",40,90);
   resh.drawString("Постановка задачи - профессор Ижуткин Виктор Сергеевич. ",40,120);
