...
/////////////////////////////////////////////////////////////////////////////
        public static void go() {
                switch (step) {
                  ...
                  case 0:
                                c = "n{}";
                                c += "При нажатии на кнопку \"Обновить\" происходит смена вариантаn{}"+
                                    "функиии (меняётся коэффициенты).";
                                nextStep = step + 1;
                                break;
                      case 1:

                                c = "  Задаём x^{0} = (" + f.x1Str() + "; " + f.x2Str() +"), \u03b5_{1} = " + f.eps1Str()+" > 0 ,"  +
                                  " \u03BC^{0} = "+f.Vektor[k]+",n{} M =" +M+" - предельное  число "+
                                    " итераций. "+
                        ...
                        case 2:
                                ...
                                break;
                        case 3:
                                c = "Найдем матрицу Гессе: n{}H(x) = #{f_{x_{1}x_{1}}}{f_{x_{1}x_{2}}}{f_{x_{2}x_{1}}}{f_{x_{2}x_{2}}} = " +
                                     f.gesseStr() + "";
                                nextStep = step + 1;
                                break;
                        case 4:
                               c = "  Вычислим k = "+k+", \u03BC^{k} = \u03BC^{"+k+"} = "+f.Vektor[k]+"";
                                 nextStep = step + 1;
                                    break;

                        case 5:
                           c = "Вычислим градиент в точке x^{" + k + "}: \u2207f(x^{" + k + "}) = #{" + f.g1Str() + "}{" +
                                        f.g2Str() + "}";
                                nextStep = step + 1;
                                break;
                        case 6:
                                c = "Проверим  критериЙ окончания: \u2225\u2207f(x^{" +
                                     k + "})\u2225 \u2264 \u03b5_{1}; ";
                                if (f.gNorm() <= f.eps1) {
                                        c += f.gNormStr() + " \u2264 " + f.eps1Str() + ",n{} условие выполняется, n{}"+
                                       // nextStep = 7;
                                        " значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +"}e{} - n{}искомое приближение к решению.";
                                 nextStep =-1;

                                }
                                else {
                                      if (slozhnost >= 1) {
                                        c +=f.gNormStr() + " > " + f.eps1Str() + ",n{} условие не выполняется. n{}Таким образом %{переходим к следующему шагу}{найдено решение} n{}!{choice}{1}";
                                      } else {
                                        c +=f.gNormStr() + " > " + f.eps1Str() + ",n{} условие не выполняется, то переходим к следующему шагу";
                                      }
                                        nextStep = step + 1;
                                }
                                break;
                        case 7:
                             ...
                             {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
                                 " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                                   "}e{} - n{}искомое приближение к решению.";
                                   nextStep =-1;
                        ...
                        case 9:
                              ...
                              c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(k);
                                nextStep = step + 1;
                                break;
                      case 10:
                               c = "  Вычисляем значение обратной матрицы : n{}"+
                    ...
                    case 13:
                     ...
                     if (f.dFuncNorm_a() < f.dFuncNorm_b()) {
                       ...
                       " Условия выполняется, поэтому полагаем k = k+1 = "+k+"+1 = "+(k+1)+" ,n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[k]/2 +
                       " и переходим к шагу 5.";
                   // " n{}Данное условие на проверку %{близости значений функций}{близости итерационных точек} n{}!{choice}{1}";
                   ...
                   " Условия не выполняется, поэтому полагаем n{}\u03BC^{"+k+"} = 2 * \u03BC^{"+k+"} = 2 * "+f.Vektor[k]/2 +" = "+(2*f.Vektor[k]/2)+
  ...
  // 2         //////////////////////////////////////////////////////////
                 ...
                 case 15:
                         c = "Проверим  критериЙ окончания: \u2225\u2207f(x^{" +
                              k + "})\u2225 \u2264 \u03b5_{1}; ";
                         if (f.gNorm() <= f.eps1) {
                                 c += f.gNormStr() + " \u2264 " + f.eps1Str() + ",n{} условие выполняется,"+
                                 " значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +"}e{} - n{}искомое приближение к решению.";
                                 nextStep =-1;
                         }
                         else {
                                  ...
                                  case 68:
                                    ...
                                    f.setx1x2(f.x1New(k), f.x2New(k));
                                        ...
                                        nextStep = step +1;
                                         ...
                                         c = "  f(x^{" + (k + 1) + "}) = " +f.dFuncNormStr_a() + " > "+ f.dFuncNormStr_b() + " = f(x^{" + k + "})"+".n{}" +
                                        " Условия не выполняется, поэтому полагаемn{} \u03BC^{"+k+"} = 2 * \u03BC^{"+k+"} = 2 * "+f.Vektor[k]/2 +" = "+(2*f.Vektor[k]/2)+
                                      " n{}и переходим к шагу 9.";
                                    //f.setx1x2(f.x1New(), f.x2New());
                                    // nextStep = 9;
                                   nextStep = step +1;
  ...
  nextStep = -1;
    ...
    /*                                                  case 230:
                                                                ...
                                                                case 231:
                                                                        ...
                                                                        if (f.gNorm() <= f.eps1) {
                                                                                c += f.gNormStr() + " \u2264 " + f.eps1Str() + ",n{} условие выполняется, "+
                                                                                " значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +"}e{} - искомое приближение к решению.";
                                                                              nextStep =-1;
                                                                        }
                                                                        else {
                                                                              if (slozhnost >= 1) {
...
*/
    /*       case 257:
                 ...
                 default:
                  c = "неизвестнsй науке текст блин - какая пи-пи-пи-пи-пи!!";
...
////////////////////////////////////////// FIX 'x1[variant]' ...
              ...
              // g2d.setColor(ColorSolutionText);
                Stringer_un.drawString("- начальная точка", xOff+xLen+18+100, yOff+15+4);
                Stringer_un.drawString("- малое число", xOff+xLen+100, yOff+15+19);
                Stringer_un.drawString("M = "+M+ "  - число итераций", xOff+xLen+28, yOff+42+10);
                Stringer_un.drawString("\u03BC^{0} = "+m+ " -положительное число", xOff+xLen+28, yOff+42+30);
//в формулах
...
//в формулах
              ...
              //Stringer_un.drawString("k \u2265 M         (E - единичная матрица)", xOff+xLen+28+5, yOff+69+20+140);
    ...
    ////////////////////////////////////////////////////////////////////////////
                if(step==9 && slozhnost==3){
                  ...
                  g.drawString("Вычисление производиться до трех знаков после запятой",100,400);
                }
                ...
                if(nextStep == -1 && showresult) {
                  ...
                  g.drawString("Ваш результат:",x1+100, y1+20);
                  ...
                  g.drawString("Количество вопросов: " + (slozhnost * k) + "",x1+30,y1+40);
                  g.drawString("Количество правильных ответов: " + (Stringer_un._numCalcChoice + Stringer_un._numCalcMist),x1+30,y1+60 );
                  g.drawString("Количество допущенных ошибок:",x1+30,y1+80);
                  g.setColor(Color.lightGray);
                  g.drawString(" - на знание:  0 ",x1+50,y1+100);
                  g.setColor(ColorSolutionText);
                  g.drawString(" - на понимание:    " + (numChoice - Stringer_un._numCalcChoice-1) ,x1+50,y1+120);
                  if (slozhnost == 3) {
                    g.drawString(" - на вычисление:   " + (numMist - Stringer_un._numCalcMist) ,x1+50,y1+140);
                  }
                  if (slozhnost != 3) {
                    g.drawString(" - на вычисление:  0 ",x1+50,y1+140);
                  }
                  g.setColor(Color.lightGray);
                  g.drawString(" - на применение:  0 ",x1+50,y1+160);
                  g.setColor(ColorSolutionText);
                  g.drawString("Результат выполнения упражнения: " + percents + "%",x1+30,y1+180);
                }
                g2d.setColor(ColorSolutionText);
//nadpisi
                g2d.setFont(FontSubTitle);
                g2d.setColor(ColorSubTitle);
                g2d.drawString("Формулы: ", xOff+xLen+18+7, yOff+57+40-3);
                g2d.drawString("Графическая иллюстрация: ", 7, yOff-3);
                g2d.drawString("Величины: ", xOff+xLen+18+7, yOff-3);
                g2d.drawString("Масштаб: ", xOff+xLen+18+7, yOff+57+20+130+25-3+60);
                g2d.drawString("Решение: ", 7, yOff+yLen+20-3);
                g2d.drawString("Задача: ", 7, -35);
                g2d.setFont(FontSolutionText);
                g2d.setColor(ColorSolutionText);
                g2d.drawString("Найти минимум функции f(x) методом Марквардта", 65, -35);
//pishet funktiu
                ...
                g.setColor(ColorSolutionText);
                if(step==0)Stringer_un.drawString("Уровень сложности: "+slozhnost,180,yOff+yLen+20+80);
}
//system koordinat
