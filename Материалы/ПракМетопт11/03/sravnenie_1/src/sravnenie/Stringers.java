...
public    Font FontSubTitle= new java.awt.Font("Dialog", 3, 12);//Шрифт
  //Поле "Решение":
public    Color ColorSolutionText = new Color(0,0,80);//Цвет текста
public    Font FontSolutionText= new java.awt.Font("Dialog", 1, 14);//Шрифт текста
public    Font FontSolutionIndex= new java.awt.Font("Dialog", 1, 11);//Шрифт индексов
public    Color ColorSolutionNumber = new Color(180,0,60);//Цвет номера шага
public    Font FontSolutionNumber = new java.awt.Font("Dialog", 3, 16);//Шрифт номера шага
  //Поле "Формулы", "Величины":
public    Color ColorFormulaText = new Color(0,0,80);//Цвет текста
public    Font FontFormulaText= new java.awt.Font("Dialog", 1, 12);//Шрифт текста
public    Font FontFormulaIndex= new java.awt.Font("Dialog", 1, 11);//Шрифт индексов
  //Поле "Графическая иллюстрация":
public    Color ColorGraficXY = new Color(50,50,50);//Цвет системы координат
public    Font FontGraficXY= new java.awt.Font("Dialog", 1, 10);//Шрифт надписей делений на системе кординат
public    Font FontGraficText= new java.awt.Font("Dialog", 1, 12);//Шрифт текста
public    Font FontGraficIndex= new java.awt.Font("Dialog", 1, 11);//Шрифт индексов
public    Font FontDialog_1_10= new java.awt.Font("Dialog", 1, 10);//
public    Font FontDialog_1_11= new java.awt.Font("Dialog", 1, 11);//
    //шрифт подписи графиков (напр. x^2,f(x); шрифт x и шрифт 2)
  //Поле "Задача":
public    Color ColorTaskText = new Color(0,0,80);//Цвет системы координат
public    Font FontTaskText= new java.awt.Font("Dialog", 1, 14);//
public    Font FontTaskIndex= new java.awt.Font("Dialog", 1, 11);//
        ...
        private static int drawSubstring(String s, int x, int y, int sy) {
                ...
                while (true) {
                        if ((s.indexOf("#", index) != -1 && s.indexOf("#", index) < s.indexOf("n{}", index)) ||
                                ...
                                // следующая строчка отвечает за отступ по y
                                if (!adj[sy]) {
                                        y += r.getHeight() / 2 + 2;
                                        adj[sy] = true;
                                }
                        ...
                        if (sym == 'n') {
                                ...
                                sh = new QuadCurve2D.Double(x, ys1, x + 5, (ys1 + ys2) / 2, x, ys2);
                                g2d.draw(sh);
                                x += 6;
                        }
                        else if (sym == '?') {
                ...
                if (c == buttons[0] || c == buttons[1]) {
                        ...
                        if (atext1.trim().toLowerCase().equals("field")) {
                                ...
                                if (fields[0].getText() == "" || fields[1].getText() == "") {
                                        ...
                                        if(userAnswer < 0 && lastErrorStep != DrawPanels.step) {
                                                mist_calc[DrawPanels.step]++;
                                                lastErrorStep = DrawPanels.step;
                                        }
                                }
                        }
                        else {
